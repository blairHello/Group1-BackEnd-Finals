...
<body>
    <br>
    <br>
    <img src="../assets/water-bg-header.png" alt="Header Image" class="header-image">

    <div class="container my-5">
        <h2>Water  <i class="fas fa-tint"></i></h2>
        <p>Track your hydration!</p>
        <br>

        <a class="btn btn-primary" href="water_create.php" role="button"><i class="fas fa-plus"></i>  Add Entry</a>
        <br>
        ...
        <table class="table">
            <thead>
                <tr>
                    <th>Date</th>
                    <th>Time of Water Intake</th>
                    <th>Glasses of Water</th>
                    <th>Quantity of Water Intake in ml</th>
                    <th>Total Water Intake in L</th>
                    <th>Action</th>
                </tr>
            </thead>
            <tbody>
                ...
                // Read all rows from the water tracker table
                $sql = "SELECT * FROM tb_water_tracker";
                ...
                while($row = $result->fetch_assoc()){
                    ...
                    $totalWaterIntakeL = $row['quantity_of_water_ml'] / 1000;

                    echo "
                        <tr>
                            <td>{$row['date']}</td>
                            <td>{$row['time_of_water_intake']}</td>
                            <td>{$row['glasses_of_water']}</td>
                            <td>{$row['quantity_of_water_ml']}</td>
                            <td>$totalWaterIntakeL</td>
                            <td>
                            <a class='btn btn-edit btn-sm' href='water_edit.php?id={$row['id']}'>Edit</a>
                            <a class='btn btn-delete btn-sm' href='water_delete.php?id={$row['id']}' onclick='return confirm(\"Are you sure you want to delete this entry?\")'>Delete</a>
                            </td>
                        </tr>
                    ";
                }
