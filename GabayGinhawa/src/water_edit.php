...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    // GET METHOD: SHOW DATA OF WATER ENTRY
    if (!isset($_GET["id"])) {
    ...
    // READ ROW OF SELECTED WATER ENTRY FROM DB TABLE
    $sql = "SELECT * FROM tb_water_tracker WHERE id=$id AND user_id=$user_id";
    ...
    $date = $row["date"];
    $time_of_water_intake = $row["time_of_water_intake"];
    $glasses_of_water = $row["glasses_of_water"];
    $quantity_of_water_ml = $row["quantity_of_water_ml"];
} else {
    // POST METHOD: UPDATE DATA OF WATER ENTRY
    $id = $_POST["id"];
    $date = $_POST["date"];
    ...
    do {
        if (empty($id) || empty($date) || empty($time_of_water_intake) || empty($glasses_of_water) || empty($quantity_of_water_ml)) {
            $errorMessage = "All the fields are required.";
            break;
        }

        // UPDATE WATER ENTRY IN DATABASE
        $sql = "UPDATE tb_water_tracker SET date = '$date', time_of_water_intake = '$time_of_water_intake', glasses_of_water = '$glasses_of_water', quantity_of_water_ml = '$quantity_of_water_ml' WHERE id = $id AND user_id = $user_id";
        $result = $connection->query($sql);
        ...
        $successMessage = "Water entry updated.";
        header("location: water.php");
        exit;
    } while (false);
}
...
<br>
    ...
    <br>
<div class="container">
    <h2 class="heading">Edit Water Entry</h2>
        ...
        <form method="post" action="water_edit.php">
