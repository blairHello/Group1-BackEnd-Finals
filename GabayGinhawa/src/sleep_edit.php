...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    // GET METHOD: SHOW DATA OF SLEEP ENTRY
    if (!isset($_GET["id"])) {
    ...
    // READ ROW OF SELECTED SLEEP ENTRY FROM DB TABLE
    $sql = "SELECT * FROM tb_sleep_tracker WHERE id=$id AND user_id=$user_id";
    ...
    $date = $row["date"];
    $time_of_sleep = $row["time_of_sleep"];
    $wake_up_time = $row["wake_up_time"];
    $quality_of_sleep = $row["quality_of_sleep"];
} else {
    // POST METHOD: UPDATE DATA OF SLEEP ENTRY
    $id = $_POST["id"];
    $date = $_POST["date"];
    ...
    do {
        if (empty($id) || empty($date) || empty($time_of_sleep) || empty($wake_up_time) || empty($quality_of_sleep)) {
            $errorMessage = "All the fields are required.";
            break;
        }

        // UPDATE SLEEP ENTRY IN DATABASE
        $sql = "UPDATE tb_sleep_tracker SET date = '$date', time_of_sleep = '$time_of_sleep', wake_up_time = '$wake_up_time', quality_of_sleep = '$quality_of_sleep' WHERE id = $id AND user_id = $user_id";
        $result = $connection->query($sql);
        ...
        $successMessage = "Sleep entry updated.";
        header("location: sleep.php");
        exit;
    } while (false);
}
...
<div class="container">
    <h2 class="heading">Edit Sleep Entry</h2>
        ...
        <form method="post" action="sleep_edit.php">
