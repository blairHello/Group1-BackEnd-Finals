...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    // GET METHOD: SHOW DATA OF ACTIVITY ENTRY
    if (!isset($_GET["id"])) {
    ...
    // READ ROW OF SELECTED ACTIVITY ENTRY FROM DB TABLE
    $sql = "SELECT * FROM tb_activity_tracker WHERE id=$id AND user_id=$user_id";
    ...
    // POST METHOD: UPDATE DATA OF ACTIVITY ENTRY
    if (!isset($_POST["id"])) {
        header("location: activity.php");
        exit;
    }
    ...
    do {
        if (empty($id) || empty($date) || empty($type_of_activity) || empty($intensity_level) || empty($duration) || empty($calories_burned)) {
            $errorMessage = "All the fields are required.";
            break;
        }

        // UPDATE ACTIVITY ENTRY IN DATABASE
        $sql = "UPDATE tb_activity_tracker SET date = '$date', type_of_activity = '$type_of_activity', intensity_level = '$intensity_level', duration = '$duration', calories_burned = '$calories_burned' WHERE id = $id AND user_id = $user_id";
        $result = $connection->query($sql);
        ...
        $successMessage = "Activity entry updated.";
        header("location: activity.php");
        exit;
    } while (false);
}
