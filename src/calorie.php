...
<body>
    <br>
    <br>
    <img src="../assets/calorie-bg-header.png" alt="Header Image" class="header-image">

    <div class="container my-5">
        <h2>Calorie  <i class="fas fa-utensils"></i></h2>
        <p>Track your calorie intake!</p>
        <br>

        <a class="btn btn-primary" href="calorie_create.php" role="button"><i class="fas fa-plus"></i>  Add Entry</a>
        <br>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                // Read all rows from the calorie tracker table
                $sql = "SELECT * FROM tb_calorie_tracker";
                ...
                while($row = $result->fetch_assoc()){
                    echo "
                        <tr>
                            <td>{$row['date']}</td>
                            <td>{$row['time_of_meal']}</td>
                            <td>{$row['food_name']}</td>
                            <td>{$row['meal_type']}</td>
                            <td>{$row['calories']}</td>
                            <td>
                            <a class='btn btn-edit btn-sm' href='calorie_edit.php?id={$row['id']}'>Edit</a>
                            <a class='btn btn-delete btn-sm' href='calorie_delete.php?id={$row['id']}' onclick='return confirm(\"Are you sure you want to delete this entry?\")'>Delete</a>
                            </td>
                        </tr>
                    ";
                }
